...
serve(async (req) => {
  ...
  try {
    ...
    // In production, you may want to validate the API key
    ...
    const activeStrategies = await supabase.from('strategies').select('*', {
      count: 'exact',
      head: true
    }).eq('status', 'active');
    const totalVolume = tradesVolume.data?.reduce((sum, t)=>sum + (t.total || 0), 0) || 0;
    const buyTrades = await supabase.from('trades').select('*', {
      count: 'exact',
      head: true
    }).eq('side', 'buy');
    const sellTrades = await supabase.from('trades').select('*', {
      count: 'exact',
      head: true
    }).eq('side', 'sell');
    const stats = {
