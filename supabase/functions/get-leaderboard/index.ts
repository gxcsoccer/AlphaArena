...
serve(async (req)=>{
  ...
  try {
    ...
    // Get all strategies
    const { data: strategies, error: strategiesError } = await supabase.from('strategies').select('id, name, status');
    if (strategiesError) {
    ...
    for (const strategy of strategies || []){
      ...
      const { data: trades } = await supabase.from('trades').select('side, total, price, quantity').eq('strategy_id', strategy.id);
    ...
    const sortKey = sortBy === 'sharpeRatio' ? 'sharpeRatio' : sortBy === 'maxDrawdown' ? 'maxDrawdown' : sortBy === 'totalPnL' ? 'totalPnL' : sortBy === 'winRate' ? 'winRate' : sortBy === 'totalVolume' ? 'totalVolume' : 'roi';
