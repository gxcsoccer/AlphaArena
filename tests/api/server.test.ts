...
describe('APIServer', () => {
  ...
  describe('WebSocket Events', () => {
    ...
    it('should subscribe to strategy room', (done) => {
      const client = SocketIOClient(`http://localhost:${testPort}`, {
        transports: ['websocket'],
      });

      client.on('connect', () => {
        client.emit('subscribe:strategy', 'test-strategy-123');
    ...
    it('should subscribe to symbol room', (done) => {
      const client = SocketIOClient(`http://localhost:${testPort}`, {
        transports: ['websocket'],
      });

      client.on('connect', () => {
        client.emit('subscribe:symbol', 'BTC/USDT');
    ...
    it('should broadcast trade event', (done) => {
      const client = SocketIOClient(`http://localhost:${testPort}`, {
        transports: ['websocket'],
      });
      ...
      client.on('connect', () => {
        ...
        // Broadcast the trade
        server.broadcastTrade(tradeData as any);
      });
    ...
    it('should broadcast portfolio update', (done) => {
      const client = SocketIOClient(`http://localhost:${testPort}`, {
        transports: ['websocket'],
      });
      ...
      client.on('connect', () => {
        ...
        client.on('portfolio:update', (data) => {
          expect(data.id).toBe('portfolio-123');
          expect(data.baseBalance).toBe(1.5);
          client.disconnect();
          done();
        });

        server.broadcastPortfolioUpdate(portfolioData as any);
      });
    ...
    it('should broadcast strategy tick to specific room', (done) => {
      const client = SocketIOClient(`http://localhost:${testPort}`, {
        transports: ['websocket'],
      });

      client.on('connect', () => {
        // Subscribe to the strategy room first
        client.emit('subscribe:strategy', 'strategy-123');
        ...
        client.on('strategy:tick', (data) => {
          expect(data.strategyId).toBe('strategy-123');
          expect(data.timestamp).toBeDefined();
          client.disconnect();
          done();
        });
        ...
        // Broadcast the tick
        setTimeout(() => {
          server.broadcastStrategyTick('strategy-123', tickData);
        }, 100);
      });
